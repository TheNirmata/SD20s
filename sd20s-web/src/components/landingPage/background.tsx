import Image from 'next/image';
import imageA from '../../../public/static-images/imageA.png';
import imageB from '../../../public/static-images/imageB.png';
import digitalCam from '../../../public/static-images/digital-cam.png';
import inDigitalCamImage from '../../../public/static-images/digitalCam-image.png';
import titleText from '../../../public/titleText.svg';
const Background = () => {
  return (
    <div className='w-[336px]'>
      <div className='relative z-10 translate-x-7'>
        <Image 
        alt='image-A'
        src={imageA}
        className='w-[336px] h-[316px] object-cover translate-x-12 -translate-y-5'
        />
      </div>
      <div className='absolute z-10 left-3 -translate-x-1 -translate-y-20 mt-9'>
        <Image 
        alt='title-text'
        src={titleText}
        className='w-[183.76px] h-[133.12px]'
        />
      </div>

      <div className='absolute z-5 translate-y-16 -translate-x-7'>
        <Image 
        alt='image-B'
        src={imageB}
        className='w-[182.76px] h-[220.22px]'
        />
      </div>
      <div className='w-[423.8px] h-[228.53px] absolute z-5 -right-7 translate-y-24 translate-x-20'>
        <Image 
        alt='digital-cam'
        src={digitalCam}
        className='w-96 h-30'
        />
      </div>
      <div>
        <div className='relative -z-10 top-20 left-20 translate-y-44 translate-x-5 rotate-19'>
          <Image 
          alt='digital-cam-image'
          src={inDigitalCamImage}
          className='w-[270.13px] h-[230.74px]'
          />
        </div>
      </div>
    </div>

  );
};

export default Background;